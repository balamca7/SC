...
class SC_subjectMaster extends MY_Model {
	...
	public function addsubCategory($userdata) {
		...
		if ($result) {
			return "subCategory Already Exist";
		} else {
			$user = array (
					'name' => $userdata ['name'],
	...
	public function getaddsubCategory($userdata) {
		$this->_table = 'subcategory';
		return $this->db->select ( 'id,name,category_id,isActive' )->from ( $this->_table )->where ( 'id', $userdata )->get ()->row ();
	}
	...
	public function getsubCategory() {
		$this->_table = 'subcategory';
		return $this->db->select ( 'id,name,category_id,isActive' )->from ( $this->_table )->get ()->result ();
	}
