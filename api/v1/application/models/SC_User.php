...
class SC_User extends MY_Model {
    ...
    public function authorize($userData) {
        $result = $this->db->select('id, user_id, f_name, l_name, CONCAT(f_name, " " ,l_name) AS name, email_1 AS email, isFirstLogin AS returningUser, imagepath As image')
                        ->from($this->_table)
                        ->where('(user_id="' . $userData['username'] . '" OR email_1="' . $userData['username'] . '")')
