...
if ($action == 'request') {
	...
	if(!empty($chatroommode)){
		...
		if($optionalmessage == 0){
			$response = sendMessage($to,"CC^CONTROL_mobileAction=\"javascript:jqcc.ccavchat.accept('".$userid."','".$grp."');\"|avchat_webaction=initiate|".$grp."|".$caller,1);
...
if ($action == 'accept') {
	...
	sendMessage($to,"CC^CONTROL_mobileAction=\"javascript:jqcc.ccavchat.accept_fid('".$userid."','".$grp."');\"|avchat_webaction=acceptcall|".$grp,1);
	if (!empty($_REQUEST['callback'])) {
		header('content-type: application/json; charset=utf-8');
