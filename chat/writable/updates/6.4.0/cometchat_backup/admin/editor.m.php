...
function index() {
	global $body, $navigation, $ts, $currentversion, $settings, $writable, $client;
	...
	if(!empty($_REQUEST['customjs'])){
		...
		if((!empty($settings['enable_customjs']) && $settings['enable_customjs']['value'] == 1) || $isenabled == 1){
			$hiddenfield = '<div class="row col-md-12" style="padding-top:5px;">Enable custom JS &nbsp;&nbsp;<input type="radio" name="enable" value="1" checked/>&nbsp;Yes&nbsp;<input type="radio" name="enable" value="0"/>&nbsp;No&nbsp;<input type="hidden" name="hiddenfield" value="customjs"/></div>';
		}else{
			$hiddenfield = '<div class="row col-md-12" style="padding-top:5px;">Enable custom JS &nbsp;&nbsp;<input type="radio" name="enable" value="1" />&nbsp;Yes&nbsp;<input type="radio" name="enable" value="0" checked />&nbsp;No&nbsp;<input type="hidden" name="hiddenfield" value="customjs"/></div>';
		}
		...
		if((!empty($settings['enable_customcss']) && $settings['enable_customcss']['value'] == 1) || $isenabled == 1 ){
			$hiddenfield = '<div class="row col-md-12" style="padding-top:5px;">Enable custom CSS &nbsp;&nbsp;<input type="radio" name="enable" value="1" checked/>&nbsp;Yes&nbsp;<input type="radio" name="enable" value="0"/>&nbsp;No&nbsp;<input type="hidden" name="hiddenfield" value="customcss"/></div>';
		}else{
			$hiddenfield = '<div class="row col-md-12" style="padding-top:5px;">Enable custom CSS &nbsp;&nbsp;<input type="radio" name="enable" value="1" />&nbsp;Yes&nbsp;<input type="radio" name="enable" value="0" checked />&nbsp;No&nbsp;<input type="hidden" name="hiddenfield" value="customcss"/></div>';
		}
	}else{
		if(!empty($client)) {
			    ...
			    <div class="card">
			      	<div class="card-header">
			        	...
			        	<select onchange="window.location='?module=editor'+this.value+'&ts={$ts}'" class="form-control" style="width:200px;float:right;">
				        	{$addintegration}
				        	<option {$op2} value="&customjs=1">Custom JS</option>
