...
function getRecentList() {
	...
	if(!empty($_REQUEST['initialize'])) {
		...
		$sql = ("select cr1.* from cometchat_chatroommessages cr1 left join cometchat_chatroommessages cr2 on cr1.chatroomid = cr2.chatroomid and cr1.sent < cr2.sent where cr2.sent is null");
...
function fetchMessages() {
	...
	while ($chat = mysqli_fetch_assoc($query)) {
		...
		if (in_array('translate',$trayicon) && (!empty($_REQUEST[$cookiePrefix.'rttlang'])||!empty($_COOKIE[$cookiePrefix.'rttlang'])) && $self == 0 && $old == 0 && strpos($chat['message'],'CC^CONTROL_') === false) {
