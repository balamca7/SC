...
function sendMessage($to,$message,$dir = 0,$type = '') {
	...
	if (!empty($to) && isset($message) && $message!='' && $userid > 0) {
		...
		if($disableRecentTab == 0) {
			$convo_hash = md5($userid<$to? md5($userid).md5($to) : md5($to).md5($userid));
	...
	if (!empty($broadcast) && $userid > 0) {
		...
		if (USE_COMET == 1) {
			...
			for ($i=0; $i < $sizeof_broadcast; $i++) {
				...
				$response = array("id" => $insertedid+$i, "m" => $broadcast[$i]['message'], "from"=> $broadcast[$i]['to'], "direction" => $broadcast[$i]['dir'], "localmessageid" => $broadcast[$i]['localmessageid']);
