...
$botcontent = json_encode($botcontent);
$botlist = json_encode($botlist);
$popoutmode = 0;
if(empty($userid) || 1){
	echo <<<EOD
	<!DOCTYPE html>
	...
	<head>
		<style type="text/css">
			...
			.cometchat_nobots{
				...
				font-size: {$themeSettings['font_size_embed']};
				line-height: 1.3em;
				padding: 10px;
		...
		<script type="text/javascript">
			jqcc(document).ready(function(){
				...
				jqcc('.cometchat_botlist').live('click',function(){
					...
					jqcc('.cometchat_bots_wrapper').html(botcontent['_'+botid]);
					...
					jqcc("#bots_window",parent.document).find("#cometchat_bot_title_text").text(botlist['_'+botid]['n']);
					...
					jqcc("#cometchat_bots_popup",parent.document).find(".cometchat_userstabtitletext").text(botlist['_'+botid]['n']);
					jqcc("#cometchat_bots_popup",parent.document).find(".cometchat_userstabtitletext").css('margin-left','10px');
				});
				jqcc('.cometchat_bots_wrapper').slimScroll({height: 'auto'});
	...
	<div style="background: #FFF;font-family: Tahoma,Verdana,Arial,'Bitstream Vera Sans',sans-serif;font-size: 16px;height: 100vh;" class="cometchat_bots_wrapper">
		{$bothtml}
	</div>
	<body>
	</html>
