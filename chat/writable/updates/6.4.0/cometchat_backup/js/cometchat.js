...
(function($){
	$.cometchat = $.cometchat||function(){
					...
					success: function(data){
						...
						if(typeof(offlinemessagesqueue) != 'undefined' && !jqcc.isEmptyObject(offlinemessagesqueue)) {
							jqcc.each(offlinemessagesqueue,function(key,value){
								if(!jqcc.isEmptyObject(value)) {
									if(value.hasOwnProperty('chatroommode') && value.chatroommode == 1) {
							...
							if((chatboxstatesarray[next] == '' || typeof(chatboxstatesarray[next])==="undefined") || chatboxstatesarray[next].split(/\|/)[0] != ccvariable.chatBoxOrder[next]){
								if(typeof(ccvariable.chatBoxOrder[next]) != 'undefined'){
									if(ccvariable.chatBoxOrder[next].charAt(0)=='_'){
										...
										jqcc.crdocked.closeChatroom(key,1);
									}else{
										jqcc.docked.closeChatbox(ccvariable.chatBoxOrder[next],1);
									}
									if(i != 0)
										next = next;
									else if(i == 0)
		...
		arguments.callee.chatboxKeydownSet = function(id, message, callbackfn,localmessagekey){
			var localmessageid = jqcc.cometchat.updateOfflinemessages(id, message, localmessagekey);
		...
		arguments.callee.updateOfflinemessages = function(id, message, localmessagekey,chatroommode){
			if(id != '' && id != 'undefined' && message != '' && message != 'undefined') {
				var localmessageid = '';
				var offlinemessagequeue = jqcc.cometchat.getFromStorage('offlinemessagequeue');
				if(typeof(localmessagekey) != 'undefined' && localmessagekey != "") {
					localmessageid = localmessagekey;
				}else{
					if(!jqcc.isEmptyObject(offlinemessagequeue)) {
						localmessageid = jqcc.cometchat.getFromStorage('offmsgcounter');
						ccvariable.localmessageid = parseInt(localmessageid['lmid'].replace("_",""));
					}
					ccvariable.localmessageid++;
					localmessageid = '_'+ccvariable.localmessageid;
					jqcc.cometchat.updateToStorage('offmsgcounter',{'lmid':localmessageid});
				}
				var messageObj = {'id':id,'message':message};
				if(typeof(chatroommode) != 'undefined' && chatroommode != '') {
					messageObj['chatroommode'] = chatroommode;
				}

				offlinemessagequeue[localmessageid] = messageObj;
