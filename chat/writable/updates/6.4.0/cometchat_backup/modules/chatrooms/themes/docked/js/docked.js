        ...
        return {
                ...
                addChatroomMessage: function(incoming){
                    ...
                    if(incoming.hasOwnProperty('localmessageid')){
                        localmessageid = incoming.localmessageid;
                    }
                    ...
                    if(message != '') {
                        ...
                        if($("#cometchat_groupmessage_"+incomingid).length > 0) {
                            ...
                            if($('#cometchat_groupmessage_'+localmessageid).length > 0){
                                $('#cometchat_groupmessage_'+localmessageid).replaceWith(temp);
                                $("#cometchat_chatboxseen_"+localmessageid).remove();
                            } else {
                ...
                loadRoom: function(clicked,id,minimized,unreadmsgcount) {
                    ...
                    cometchat_group_popup.find('.cometchat_plugins_dropdown').click(function(e){
                        ...
                        if(jqcc().slimScroll){
                            ...
                            cometchat_group_popup.find('.cometchat_popup_plugins').find('.slimScrollDiv').css({'box-shadow': '0px 5px 8px -3px #D1D1D1','margin-top':'3px' });
                        }
                    });
                ...
                loadChatroomList : function(item) {
                    ...
                    $.each(item, function(i,room) {
                        ...
                        if((room.s == 1 || jqcc.cometchat.checkModerator() == 1) && room.createdby != 0){
                            ...
                            deletegroup = '<div class="cometchat_groupdelete" title="<?php echo $chatrooms_language[58]; ?>" onclick = "javascript:jqcc.cometchat.deleteChatroom(event,'+room.id+');"><img class="hoverbraces" src="'+baseUrl+'themes/docked/images/bin.svg"></div>';
                        }

                        temp = '<div id="cometchat_grouplist_'+room.id+'" class="cometchat_grouplist" onmouseover="jqcc(this).addClass(\'cometchat_grouplist_hover\');" onmouseout="jqcc(this).removeClass(\'cometchat_grouplist_hover\');" onclick="javascript:jqcc.cometchat.chatroom(\''+room.id+'\',\''+urlencode(shortname)+'\',\''+room.type+'\',\''+room.i+'\',\''+room.s+'\',\'1\',\'1\');"><div class="cometchat_groupscontentavatar"><img class="cometchat_groupscontentavatarimage" src="'+baseUrl+'themes/'+calleeAPI+'/images/group.svg"></div><div><div class="cometchat_groupscontentname">'+longname+'</div></div>'+deletegroup+renamegroup+roomtype+usercount+'</div>';
