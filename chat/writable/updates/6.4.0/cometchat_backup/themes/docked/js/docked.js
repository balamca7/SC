        ...
        return {
            ...
            buddyList: function(item){
                ...
                $.each(item, function(i, buddy){
                    ...
                    if((buddy.s != 'offline' && settings.hideOffline == 1) || settings.hideOffline == 0){
                        buddylisttemp += group+'<div id="cometchat_userlist_'+buddy.id+'" class="cometchat_userlist" onmouseover="jqcc(this).addClass(\'cometchat_userlist_hover\');" onmouseout="jqcc(this).removeClass(\'cometchat_userlist_hover\');"><div style="display:inline-block;"><div class="cometchat_userscontentname">'+longname+'</div><div id="cometchat_buddylist_typing_'+buddy.id+'" class="cometchat_buddylist_typing"></div></div>'+statusIndicator+'</div>';
                        buddylisttempavatar += group+'<div id="cometchat_userlist_'+buddy.id+'" class="cometchat_userlist" onmouseover="jqcc(this).addClass(\'cometchat_userlist_hover\');" onmouseout="jqcc(this).removeClass(\'cometchat_userlist_hover\');"><div class="cometchat_userscontentavatar"><img class="cometchat_userscontentavatarimage" original="'+buddy.a+'"></div><div style="display:inline-block;"><div class="cometchat_userscontentname">'+longname+'</div><div id="cometchat_buddylist_typing_'+buddy.id+'" class="cometchat_buddylist_typing"></div></div>'+statusIndicator+'</div>';
                    }
            ...
            recentList: function(item){
                ...
                $.each(item, function(i, chat){
                    ...
                    if(chat.grp) {
                        ...
                        if(chat.type == 1) {
                            roomtype = '<div class="cometchat_grouptype"><img src="'+baseUrl+'themes/'+settings.theme+'/images/lock.png" class="cometchat_grouptypeimage" /></div>';
                        }
            ...
            addMessages: function(item){
                ...
                $.each(item, function(i, incoming){
                    ...
                    if(incoming.id == undefined && incoming.localmessageid != undefined && incoming.localmessageid != '') {
                        incoming.id = incoming.localmessageid;
                    }
                    ...
                    if($("#message_"+incoming.id).length>0){
                        ...
                        if(parseInt(incoming.self)==1 && (!incoming.hasOwnProperty('botid') || (typeof incoming.botid == "undefined" && incoming.botid == 0))){
                           ...
                           if((message.indexOf('<img')==-1 && message.indexOf('src')==-1) || (smileycount > 0 && smileymsg != '')){
                            ...
                            msg2 = '<div class="cometchat_chatboxmessage" id="cometchat_message_'+incoming.id+'"><div class="'+add_bg+' '+'cometchat_ts_margin cometchat_self_msg cometchat_floatR" style="'+add_style+'"><span id="message_'+incoming.id+'">'+message+'</span></div>'+add_arrow_class+' '+sentdata_box+'</div><span id="cometchat_chatboxseen_'+incoming.id+'"></span>';
                            msg =msg1+msg2;
                            addMessage = 1;
                            ...
                            msg1 = '<div class="cometchat_time cometchat_time_'+msg_date_class+' '+date_class+'" msg_format="'+msg_date_format+'">'+msg_date+'</div>';
	                        msg2 = '<div '+avchatdisplay+' class="cometchat_chatboxmessage" id="cometchat_message_'+incoming.id+'">'+selfstyleAvatar+'<div class="'+add_bg+' '+'cometchat_ts_margin cometchat_floatL" style="'+add_style+'"><span id="message_'+incoming.id+'" class="cometchat_msg">'+message+'</span></div>'+add_arrow_class+' '+sentdata_box+'</div>';
        	                msg =msg1+msg2;
                        ...
                        if($("#cometchat_message_"+incoming.localmessageid).length>0){
                            $("#cometchat_message_"+incoming.localmessageid).replaceWith(msg2);
                        ...
                        if(typeof(incoming.id) != 'undefined' && !jqcc.isNumeric(incoming.id) &&  incoming.id.indexOf('_')>-1) {
                            $("#cometchat_chatboxseen_"+incoming.id).addClass('cometchat_offlinemessage');
                        }
                    ...
                    if(settings.disableRecentTab == 0){
                        ...
                        var params = {'chatid':incoming.from,'isgroup':0,'timestamp':incoming.sent,'m':message,'msgid':incoming.id,'force':0,'del':0};
            ...
            addPopup: function(id, amount, add){
                if((jqcc.cometchat.getThemeArray('buddylistName', id)==null||jqcc.cometchat.getThemeArray('buddylistName', id)=='') && amount>0){
                    ...
                    if(add == 1){
                        amount = parseInt($('#cometchat_recentlist_'+id).attr('amount'))+parseInt(amount);
                    }
            ...
            applyChatBoxStates: function(statestoapply){
                jqcc.each(statestoapply, function(i, state){
                    ...
                    if(typeof(state.g)!="undefined" && state.g==1){
                        if(silent>0){
                            jqcc.cometchat.getChatroomDetails(id,1,1);
                        } else if(count>0){
